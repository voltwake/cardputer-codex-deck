...
// CardBridge-specific identity, topology, and compatibility declarations.
// The audio engine below is derived from BlackHole under GPLv3; keeping the
// product configuration separate makes future upstream rebases mechanical.
#include "../CardBridgeMicrophoneConfig.h"
...
static OSStatus	BlackHole_GetBoxPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioBoxPropertyTransportType:
			...
			*((UInt32*)outData) = kCardBridgeTransportType;
			*outDataSize = sizeof(UInt32);
			break;
